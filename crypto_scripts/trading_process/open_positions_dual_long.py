...
async def open_futures_trade_second(trade_id, symbol, direction, chat_id, message_id):
    try:
        ...
        error_message = f"Error opening trade ID: {trade_id}, Symbol: {symbol}, Direction: {direction}, Error: {e}, Cab: {cab}"
        await send_to_error(error_message)
        # Добавляем дополнительную информацию в лог
        logging.error(f"Trade details - Price: {price}, Quantity: {quantity}, Precision: {precision}")
        raise
