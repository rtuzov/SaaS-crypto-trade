...
def update_stop_loss(position):
    ...
    if key not in stop_loss_dict:
        stop_loss_dict[key] = basis_stop_loss
    elif unrealized_profit < threshold_switch and stop_loss_dict[key] > basis_stop_loss:
        stop_loss_dict[key] = stop_loss_dict[key]  # Эта строка эффективно ничего не делает.
    else:
        new_stop_loss = unrealized_profit * 0.95
        stop_loss_dict[key] = max(stop_loss_dict[key], new_stop_loss)

def update_anomally_sl(position):
    """
    Обновление стоп-лосса на текущий трейл.
    """
    symbol = position['symbol']
    position_amt = float(position['notional'])
    is_long = position_amt > 0
    ...
    unrealized_profit = float(position['unrealizedProfit'])
    new_stop_loss = unrealized_profit * 0.95
    stop_loss_dict[key] = new_stop_loss
