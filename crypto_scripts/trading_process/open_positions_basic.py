...
def get_asset_precision(symbol):
    try:
        if symbol in ["TONUSDT", "BSVUSDT", "MAVIAUSDT"]:
            return 1
...
async def open_futures_trade(trade_id, symbol, direction, chat_id, message_id):
    try:
        ...
        while precision >= 0:
            try:
               ...
               # Adjust quantity based on static minimum values
                ...
                ttl = 240  # Время жизни ключа в секундах (5 минут)
