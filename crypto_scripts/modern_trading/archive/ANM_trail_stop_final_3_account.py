...
logging.basicConfig(filename='trading_bot_3.log', level=logging.INFO, format='%(asctime)s - %(message)s')


# Ваши API ключ и секрет
API_KEY = 'jVDyT2v7hWXgmxfIdOOWfnUFgiyuC6nnJKdFva0vOnKBnAJduIYGJbns77sXD7si'
API_SECRET = 'Ly5Iq31vz9G4yvX5YOtnsdQv7fUtUB9pxjH6gdmbRtfjj8UI4A8RlasN9KtgTr0W'
...
INITIAL_STOP_LOSS = -25  # начальный стоп-лосс в долларах
        ...
        for position in open_positions:
            ...
            if (is_long and unrealized_profit <= stop_loss_value) or (not is_long and unrealized_profit <= stop_loss_value):
                ...
                if stop_loss_counter[key] >= 25:
                    ...
                    message = f"БЕЗ СТОПА. Закрытие позиции {symbol}, объемом {position_amt} в направлении {'LONG' if is_long else 'SHORT'} с прибылью/убытком {unrealized_profit}"
