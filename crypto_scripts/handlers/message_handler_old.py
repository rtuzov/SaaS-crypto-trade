...
async def handle_text_messages(client: Client, message: Message):
    ...
    async with SessionLocal() as session:
        try:
            text_replace, title, text_original = extract_texts_and_title(message)
            if not text_replace:
                logger.warning("No text found in message")
                return

            await check_links_and_spot(message, text_replace, text_original)
            logger.info("Processing message: %s", text_replace)

            if await check_and_handle_forwarded_message(redis_queue, message):
                return

            text_lower = text_replace.lower().replace('\n', ' ')
            cat = check_keywords(text_lower)

            if cat:
                if title is None:
                    title = "None"
                message_json = json.dumps(message_to_dict(message, text_original))
                redis_key = f"{message.chat.id}:{message.id}"
                redis_value = f"{message.date}|{message.chat.id}|{title}|{message.id}|{text_replace}|{cat}|{message_json}"

                await save_to_redis(redis_queue, redis_key, redis_value)
                await process_message(redis_sql, message, text_replace, text_lower, cat, title)

        except Exception as e:
            logger.error("Error processing message: %s: %s - %s", e, message.chat.title, text_replace)
            if redis_sql:
                async with session:
                    await save_error_message(session, message, redis_sql, title)


def extract_texts_and_title(message: Message):
    ...
    try:
        ...
        if message.text:
            ...
            text_original = message.caption
        elif message.forward_from or message.forward_from_chat:
            text_original = (message.text or '') + ' ' + (message.caption or '')
            text_original = text_original.strip()
        else:
            text_original = None

        if text_original:
            text_replace = text_original.lower().replace('\n', ' ').replace('|', '\\')
        ...
        return text_replace, title_replace, text_original
...
async def save_error_message(session, message, redis_sql, title):
    ...
    text = extract_texts_and_title(message)
...
ЧАТ: {message.chat.title}:{message.chat.id}:{message.id}
ИСХОДНИК: {text}
🎊 Выгодный сигнал:
...
Риск: Низкий
    ...
    try:
        ...
        logger.error(f"Error sending message to Telegram channels: {e}, {message.chat.title}:{message.chat.id}:{message.id}")
