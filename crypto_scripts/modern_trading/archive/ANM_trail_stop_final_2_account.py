...
logging.basicConfig(filename='trading_bot_2.log', level=logging.INFO, format='%(asctime)s - %(message)s')


# Ваши API ключ и секрет
API_KEY = 'UdHx4OzSs3by5FK0T6rMQZi1FE3pNV1mVwPUvW1ZDJAfUVLb2cuJnWUV3j3AeuJC'
API_SECRET = 'BeLZPDpkDRxRv2KLfrRKbEokQny5WTsJmSpl5Z1Ei5YsduUarhjgKVXR2Z6qFA5V'
...
PROFIT_THRESHOLD = 1  # порог прибыли для изменения стоп-лосса
TRAILING_STOP_INCREMENT = 0.5  # инкремент для трейлинг стопа

# Словарь для отслеживания стоп-лоссов
stop_loss_dict = {}

# Новый словарь для отслеживания счетчика детекций стоп-лосса
stop_loss_counter = {}
...
def check_anomaly_signal():
    try:
        with open("anomaly_signal.txt", "r") as file:
            lines = file.readlines()  # Считываем все строки файла
            if lines:  # Проверяем, есть ли строки
                last_line = lines[-1].strip()  # Обрабатываем последнюю строку
                return last_line
        return None
    except FileNotFoundError:
        # Файл не найден, сигнал не активен
        return None
...
def update_stop_loss(position):
    ...
    if key not in stop_loss_dict or unrealized_profit < threshold_switch:
        ...
        new_stop_loss = unrealized_profit * 0.95
...
def monitor_and_manage_positions():
    ...
    while True:
        ...
        for position in open_positions:
            ...
            print("COUNTER", stop_loss_counter)
            ...
            if (is_long and unrealized_profit <= stop_loss_value) or (not is_long and unrealized_profit <= stop_loss_value):
                ...
                stop_loss_counter[key] = stop_loss_counter.get(key, 0) + 1

                # Если счетчик достиг 25, закрываем позицию
                if stop_loss_counter[key] >= 25:
                    close_position(symbol, abs(position_amt), is_long)
                    message = f"ДВЕ СТОРОНЫ. Закрытие позиции {symbol}, объемом {position_amt} в направлении {'LONG' if is_long else 'SHORT'} с прибылью/убытком {unrealized_profit}"
                    logging.info(message)  # Логирование в файл
                    send_telegram_message(TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, message)

                    # Сброс счетчика после закрытия позиции
                    del stop_loss_counter[key]

        '''signal_type = check_anomaly_signal()
        print("this is in anomaly file", signal_type)
        if signal_type:
            for position in open_positions:
                symbol = position['symbol']
                position_amt = float(position['positionAmt'])
                ...
                # Если сигнал LONG, закрываем все SHORT позиции
                if signal_type == "LONG" and not is_long:
                    close_position(symbol, abs(position_amt), is_long)
                    message = f"Closed SHORT position for {symbol} due to LONG Anom."
                    logging.info(message)
                    send_telegram_message(TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, message)

                # Если сигнал SHORT, закрываем все LONG позиции
                elif signal_type == "SHORT" and is_long:
                    close_position(symbol, abs(position_amt), is_long)
                    message = f"Closed LONG position for {symbol} due to SHORT Anom."
                    logging.info(message)
                    send_telegram_message(TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, message)
