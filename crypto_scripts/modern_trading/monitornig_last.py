...
try:
    from config import api_key1, api_secret1, FIXED_VOLUME1, cab1, counter1
except ImportError as e:
    print(f"Ошибка при импорте модуля config: {e}")
    sys.exit(1)
    

from config import api_key1, api_secret1, FIXED_VOLUME1, counter1, INITIAL_STOP_LOSS1, PROFIT_THRESHOLD1, TRAILING_STOP_INCREMENT1
...
BASE_VALUE = FIXED_VOLUME1  # базовое значение позиции в долларах
INITIAL_STOP_LOSS = INITIAL_STOP_LOSS1  # начальный стоп-лосс в долларах
PROFIT_THRESHOLD = PROFIT_THRESHOLD1  # порог прибыли для изменения стоп-лосса в долларах
TRAILING_STOP_INCREMENT = TRAILING_STOP_INCREMENT1  # инкремент для трейлинг стопа
...
async def update_stop_loss(position, redis_keys, stop_loss, counter, anom, trail):
    """
...
#    basis_stop_loss_old = round((abs(position_notional) / 100) * INITIAL_STOP_LOSS, 2)
    basis_stop_loss = round(init_margin * INITIAL_STOP_LOSS, 6)

#    threshold_switch_old = round(abs(position_notional) / 100) * PROFIT_THRESHOLD

    threshold_switch = round(init_margin * PROFIT_THRESHOLD, 6)

    if unrealized_profit > threshold_switch or anom == 1  or trail == 1:
    ...
    for k in redis_keys:
        if key in k:
            ...
            print(
                f"'Basis: В Redis'{key} 'Amt:' {position_amt} 'Profit:' {unrealized_profit} 'SL:' {new_stop_loss} 'Counter:' {counter} 'Anomal:' {anom} 'Trail:' {trail}")

            await save_to_redis_open_trade(redis, k, value)
            logging.info(f"Updated Redis key {k} with value {value}")

    return new_stop_loss, position_amt, unrealized_profit, counter, anom, trail
...
async def monitor_and_manage_positions():
    while True:
        ...
        for position in open_positions:
            ...
            cab_value = str(cab1)  # Make sure it's a string if it's not already
            ...
            if matching_key:
                ...
                if value:
                    ...
                    if redis_value['counter'] >= counter1:
                        await close_position(symbol, abs(position_amt), direction)
                        message = f"{cab}: Закрытие позиции {symbol}, объемом {position_amt} в направлении {direction} по причине counter >= {counter1}, trail = {trail}, anom = {anom}, profit = {redis_value['unrealized_profit']}"
                    ...
                    new_stop_loss, position_amt, unrealized_profit, counter, anom, trail = await update_stop_loss(
                        position, redis_keys, redis_value['stop_loss'],
                        redis_value['counter'], redis_value['anom'], redis_value['trail'])
                    ...
                    if unrealized_profit <= new_stop_loss and trail == 1:
                        await close_position(symbol, abs(position_amt), direction)
                        message = f"{cab}: Закрытие позиции {symbol}, объемом {position_amt} в направлении {direction} по причине достижения стоп-лосса {new_stop_loss}, trail = {trail}, anom = {anom}, profit = {redis_value['unrealized_profit']} "
                        logging.info(message)
                        send_telegram_message(TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, message)
                        await delete_matching_redis_keys(f"*{key}*")
                        continue
                ...
                redis_keys = await fetch_and_store_redis_keys()
                await update_stop_loss(position, redis_keys, -1, 0, 0, 0)
        print('-------------------')
        time.sleep(2)
